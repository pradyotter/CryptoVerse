...
const Market = () => {
  const { cryptoData, currency, getCoininfo } = useContext(CryptoContext);
  ...
  return (
    <section className=' w-full mt-5'>
      ...
      <div
        onClick={closeModal}
        className={
          selectedCoin && openModal
            ? 'flex items-center relative blur-sm   mt-12 w-[80%] mx-auto'
            : 'flex items-center relative    mt-12 w-[80%] mx-auto'
        }
      >
        {cryptoData ? (
          <table className='w-[80%] mx-auto table-auto'>
            <thead className='capitalize   border-b '>
              <tr className='text-[1.2rem] '>
                <th className='py-3'>Asset</th>
                <th className='py-3'>Name</th>
                <th className='py-3'>Price</th>
                <th className='py-3'>Total Volume</th>
                <th className='py-3'>Market Cap Change</th>
            ...
            <tbody>
              {cryptoData.map((ele) => {
                return (
                  <tr key={ele.id} className='text-center text-[1rem] '>
                    <td className='py-4 flex items-center gap-3 justify-start  '>
                      <button className='outline-0 border-0 bg-none cursor-pointer'>
                        <svg
                          className=' fill-black hover:fill-cyan-400'
                          width='30'
                          height='30'
                          viewBox='0 0 30 30'
                          fill='cyan'
                      ...
                      <div
                        ...
                        className='flex ml-2 items-center gap-2 cursor-pointer '
                      >
                        <img
                    ...
                    <td className='py-4'>{ele.name}</td>
                    <td className='py-4'>
                      {new Intl.NumberFormat('en-IN', {
                        style: 'currency',
                        currency: currency,
                      }).format(ele.current_price)}
                    </td>
                    <td className='py-4'>{ele.total_volume}</td>
                    <td className='py-4'>{ele.market_cap_change_24h}</td>
                    <td
                      className={
                        ele.price_change_percentage_1h_in_currency.toFixed(2) >
                        0
                          ? 'py-4 text-green-500'
                          : 'py-4 text-red-500'
                      }
                    ...
                    <td
                      className={
                        ele.price_change_percentage_24h_in_currency.toFixed(2) >
                        0
                          ? 'py-4 text-green-500'
                          : 'py-4 text-red-500'
                      }
      ...
      {selectedCoin && openModal && <CoinDetails coinInfo={selectedCoin} />}
    </section>
  );
};
